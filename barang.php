				...
				<div class="table-responsive22">
					...
					<form action="barang.php" method="post">
						<div class="box-toole pull-right" style="margin-right: 2px;">
							...
							<a href="tbarang.php" class="btn btn-info">Tambah</a>
						</div>
						<div class="box-toole pull-right" style="margin-right: 2px;">
						...
						<div class="box-toole pull-right" style="margin-right: 2px;">
							<select name="tipe" class="form-control">
								...
								<option value="1" <?php echo $tipe === '1' ? 'selected' : "" ?>> Makanan</option>
								<option value="2" <?php echo $tipe === '2' ? 'selected' : "" ?>> Minuman</option>
							</select>
						</div>
					</form>
					...
					<table id="datatable" class="table table-bordered table-striped">
						...
						<tbody>
							...
							if ($search) {
								...
								$where .= " OR harga like '%{$search}%'" ;
								$where .= " OR deskripsi like '%{$search}%'" ;
							}
							...
							$ambildata = mysqli_query($conn,"SELECT * FROM barang where 1 = 1 ".$where);

							$row = mysqli_num_rows($ambildata);

							if ($row > 0) {
								while($tampil = mysqli_fetch_array($ambildata)){
									$status = $tampil['status'] === '1' ?  'Aktif' :  "Tidak Aktif";
									$tipe = $tampil['tipe'] === '1' ?  'Makanan' :  "Minuman";
									echo "
									<tr>
										<td><center>".$no."</center></td>
										<td><center>".$tampil['nama']."</center></td>
										<td><center>".$tampil['harga']."</center></td>
										<td><center>".$tampil['deskripsi']."</center></td>
										<td><center>". $status ."</center></td>
										<td><center>". $tipe ."</center></td>
										<td>
											<center> 
												<a href=\"deletebarang.php?id=". $tampil['id']."\" class=\"btn btn-danger btn-flat btn-xs\" data-toggle=\"modal\" data-target=\"#delete<?php echo $no; ?>\"><i class=\"fa fa-trash\"></i> Delete</a>
												<a href=\"editbarang.php?id=". $tampil['id']."\" class=\"btn btn-primary btn-flat btn-xs\"  ><i class=\"fa fa-pencil\"></i>Edit</a>
											</center>
										</td>
									</tr>";
									$no++;
